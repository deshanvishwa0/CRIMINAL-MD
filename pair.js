...
╰──────────────────────────╯`,
  contextInfo: {
    ...
    const _menuNumMap = {
      '1': `${config.PREFIX}dl`,
      '2': `${config.PREFIX}cr`,
      '3': `${config.PREFIX}tools`,
      '4': `${config.PREFIX}groupcmds`,
      '5': `${config.PREFIX}ownercmds`,
      '6': `${prefix}setting`,
      '7': `${config.PREFIX}active`
    };
    ...
    const menuHandler = async (msgUpdate) => {
      try {
        ...
        // Accept number text reply
        const replyNum = (received.message?.extendedTextMessage?.text || received.message?.conversation || '').trim();
        if (_menuNumMap[replyNum]) {
          selectedId = _menuNumMap[replyNum];
        }
...
`
  ...
  const _dlSubMap = { '1': `${config.PREFIX}song`, '2': `${config.PREFIX}video`, '3': `${config.PREFIX}fb`, '4': `${config.PREFIX}insta`, '5': `${config.PREFIX}tiktok`, '6': `${config.PREFIX}mf`, '7': `${config.PREFIX}apk`, '8': `${config.PREFIX}splotify` };
  const _dlSubHandler = async (upd) => {
    const m = upd.messages?.[0];
    if (!m || m.key.remoteJid !== sender) return;
    const txt = (m.message?.conversation || m.message?.extendedTextMessage?.text || '').trim();
    const cmd = _dlSubMap[txt];
    if (!cmd) return;
    socket.ev.off('messages.upsert', _dlSubHandler);
    await socket.sendMessage(sender, { react: { text: "⬇️", key: m.key } });
    await socket.sendMessage(sender, { text: `⚡ Type *${cmd}* to start the download.` }, { quoted: m });
  };
  socket.ev.on('messages.upsert', _dlSubHandler);
  setTimeout(() => socket.ev.off('messages.upsert', _dlSubHandler), 60000);
...
• *${config.PREFIX}img* - Image Search
• *${config.PREFIX}aiimg* - AI Image Generate
• *${config.PREFIX}font* - Fancy Text
• *${config.PREFIX}calc* - Calculator
• *${config.PREFIX}weather* - Weather
• *${config.PREFIX}git* - Git Helper
• *${config.PREFIX}boom* - Boom Effect
• *${config.PREFIX}hack* - Hack Animation
...
• *${config.PREFIX}autoreact* - Auto React
• *${config.PREFIX}setting* - Settings
• *${config.PREFIX}set* - Quick Setup
...
Type a command directly:
🤖 *Auto Commands:*
• *${config.PREFIX}autosong* | *${config.PREFIX}autottsend*
• *${config.PREFIX}autotyping* | *${config.PREFIX}autorecording*
• *${config.PREFIX}autoreact* | *${config.PREFIX}mread*

🛡️ *Anti Protection:*
• *${config.PREFIX}antiban* | *${config.PREFIX}antispam*
• *${config.PREFIX}antibug* | *${config.PREFIX}antilink*
• *${config.PREFIX}creject*

⚙️ *Bot Control:*
• *${config.PREFIX}botpresence* | *${config.PREFIX}setting*
...
┃ ╰ ➦ Typing: ${currentConfig.AUTO_TYPING || 'false'} | Recording: ${currentConfig.AUTO_RECORDING || 'false'}
    ...
    // 6. Build numbered settings menu text
    const settingMenuText = `

╭──────────────────────────╮
│  ⚙️ *Reply with a number:*
│
│  *1️⃣* ✏️ Change Bot Name
│  *2️⃣* 🌍 Public Mode
│  *3️⃣* 🔐 Private Mode
│  *4️⃣* 👥 Groups Only
│  *5️⃣* 📥 Inbox Only
│  *6️⃣* 🟢 Always Online ON
│  *7️⃣* ⚫ Always Online OFF
│  *8️⃣* ✍️ Auto Typing ON
│  *9️⃣* 🔇 Auto Typing OFF
│  *10* 👁️ Auto View Status ON
│  *11* 🙈 Auto View Status OFF
│  *12* ❤️ Auto Like Status ON
│  *13* 💔 Auto Like Status OFF
│  *14* 🚫 Auto Reject Call ON
│  *15* 📞 Auto Reject Call OFF
│  *16* 📖 Auto Read All ON
│  *17* 📪 Auto Read OFF
╰──────────────────────────╯`;

    // Setting number → command map
    const _settingMap = {
      '1': `${prefix}setbotname`, '2': `${prefix}wtype public`,
      '3': `${prefix}wtype private`, '4': `${prefix}wtype groups`,
      '5': `${prefix}wtype inbox`, '6': `${prefix}botpresence online`,
      '7': `${prefix}botpresence offline`, '8': `${prefix}autotyping on`,
      '9': `${prefix}autotyping off`, '10': `${prefix}rstatus on`,
      '11': `${prefix}rstatus off`, '12': `${prefix}arm on`,
      '13': `${prefix}arm off`, '14': `${prefix}creject on`,
      '15': `${prefix}creject off`, '16': `${prefix}mread all`,
      '17': `${prefix}mread off`
    };

    const settingSentMsg = await socket.sendMessage(sender, {
      image: { url: currentConfig.logo || config.RCD_IMAGE_PATH },
      caption: msgCaption + settingMenuText,
      footer: `powered by ${config.OWNER_NAME || 'Bot Owner'}`,
    }, { quoted: msg });

    const _settingHandler = async (upd) => {
      const m = upd.messages?.[0];
      if (!m || m.key.remoteJid !== sender) return;
      const txt = (m.message?.conversation || m.message?.extendedTextMessage?.text || '').trim();
      const cmd = _settingMap[txt];
      if (!cmd) return;
      socket.ev.off('messages.upsert', _settingHandler);
      await socket.sendMessage(sender, { react: { text: '⚙️', key: m.key } });
      await socket.sendMessage(sender, { text: `✅ Type *${prefix}${cmd.replace(prefix, '')}* to apply, or it has been noted.` }, { quoted: m });
    };
    socket.ev.on('messages.upsert', _settingHandler);
    setTimeout(() => socket.ev.off('messages.upsert', _settingHandler), 120000);
...
case 'pp': {
  try {
    ...
    const searchApi = `https://api.zanta-mini.store/api/paper/search?q=${encodeURIComponent(q)}`;
    ...
    const listener = async (update) => {
      try {
        ...
        if (isReply && ['1','2','3','4','5'].includes(text)) {
          ...
          try {
            const dlApi = `https://api.zanta-mini.store/api/paper/search?url=${encodeURIComponent(selected.link)}`;
...
`.trim();
    ...
    // 5. Send owner info as plain image message
    await socket.sendMessage(sender, {
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            caption: `🖼️ *𝐈mage 𝐒earch:* ${q}\n\n*𝐏rovided 𝐁y ${botName}*\n\n_Reply *next* for another image_`,
...
╰───────────────────────◆`;
        ...
        // 6. Message යැවීම (number reply based)
        const buttonMessage = {
        ...
        const handleTikTokSelection = async ({ messages: replyMessages }) => {
            ...
            const isPlainNumReply = sender === replyMek.key.remoteJid && ['1','2','3','4'].includes(selectedId);

            if ((isReplyToSentMsg || isPlainNumReply) && sender === replyMek.key.remoteJid) {
...
`;
            ...
            const _newsMap = { '1': `${config.PREFIX}ada`, '2': `${config.PREFIX}hiru`, '3': `${config.PREFIX}sirasa`, '4': `${config.PREFIX}itn`, '5': `${config.PREFIX}lnw`, '6': `${config.PREFIX}bbc`, '7': `${config.PREFIX}dasathalanka`, '8': `${config.PREFIX}siyatha`, '9': `${config.PREFIX}lankadeepa`, '10': `${config.PREFIX}gagana` };
            const newsMenuText = `
📰 *Reply with a number:*
*1️⃣* 🌅 Ada News
*2️⃣* 🌞 Hiru News
...
*🔟* 📦 Gagana`;
            ...
            const _newsSentMsg = await socket.sendMessage(sender, {
              image: imagePayload,
              caption: text + newsMenuText,
              footer: ` *${botName}*`
            }, { quoted: metaQuote });

            const _newsHandler = async (upd) => {
              const m = upd.messages?.[0];
              if (!m || m.key.remoteJid !== sender) return;
              const txt = (m.message?.conversation || m.message?.extendedTextMessage?.text || '').trim();
              const cmd = _newsMap[txt];
              if (!cmd) return;
              socket.ev.off('messages.upsert', _newsHandler);
              await socket.sendMessage(sender, { react: { text: '📰', key: m.key } });
              await socket.sendMessage(sender, { text: `⚡ Sending news, type *${cmd}* to get updates.` }, { quoted: m });
            };
            socket.ev.on('messages.upsert', _newsHandler);
            setTimeout(() => socket.ev.off('messages.upsert', _newsHandler), 60000);
