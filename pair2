...
const config = {
  ...
  PREFIX: '.',
  MAX_RETRIES: 3,
  GROUP_INVITE_LINK: 'https://chat.whatsapp.com/LDfplthel1c5J7Yq8sarHj?s',
...
function setupCommandHandlers(socket, number) {
  // 🔢 NUMBER-REPLY MENU SYSTEM
  // Converts WhatsApp button/list menus to plain numbered replies.
  const numberMenuState = new Map();
    ...
    try {
      if (content && Array.isArray(content.buttons) && content.buttons.length) {
        ...
        for (const btn of content.buttons) {
          // Native-flow single select: expose every row as a number.
          if (btn?.type === 4 && btn?.nativeFlowInfo?.paramsJson) {
            try {
              const params = JSON.parse(btn.nativeFlowInfo.paramsJson);
              for (const section of (params.sections || [])) {
                for (const row of (section.rows || [])) {
                  if (row?.id) items.push({ title: row.title || row.description || row.id, id: row.id });
                }
              }
            } catch (_) {}
          } else if (btn?.buttonId) {
            items.push({
              title: btn.buttonText?.displayText || btn.buttonId,
              id: btn.buttonId
            });
          }
        }
        ...
        if (items.length) {
          ...
          items.forEach((item, i) => {
            const n = String(i + 1);
            map.set(n, item.id);
            menuText += `${n}. ${item.title}\n`;
          });
          numberMenuState.set(jid, map);

          const converted = { ...content };
          delete converted.buttons;
          delete converted.headerType;

          const listText = `\n\n🔢 Reply with the number of your choice.\n\n${menuText.trim()}`;
          if (typeof converted.text === 'string') converted.text += listText;
          else if (typeof converted.caption === 'string') converted.caption += listText;
          else converted.text = listText.trim();

          return originalSendMessage(jid, converted, options);
        }
      }
    } catch (e) {
      console.warn('[NUMBER-MENU] conversion failed:', e?.message || e);
    }
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // If the user replied with a number, resolve it against the last menu sent to this chat.
    if (/^\d+$/.test(body.trim())) {
      const menu = numberMenuState.get(from);
      const mapped = menu?.get(body.trim());
    ...
    try {
      ...
      // =====================================================
// 🔢 NUMBER REPLY → COMMAND CONVERTER
// Put this DIRECTLY ABOVE: switch (command)
// =====================================================

let numberCommand = null;
...
const replyNumber = incomingText.trim();

// =====================================================
// MAIN MENU NUMBER REPLIES
// =====================================================
...
if (/^\d+$/.test(replyNumber)) {

  // Use your existing menu state if available
  if (typeof menuStage !== "undefined") {

    if (menuStage === "main") {
      numberCommand = mainMenuNumbers[replyNumber];
    }

    else if (menuStage === "download") {
      numberCommand = downloadNumbers[replyNumber];
    }

    else if (menuStage === "creative") {
      numberCommand = creativeNumbers[replyNumber];
    }

    else if (menuStage === "tools") {
      numberCommand = toolsNumbers[replyNumber];
    }

    else if (menuStage === "group") {
      numberCommand = groupNumbers[replyNumber];
    }

    else if (menuStage === "owner") {
      numberCommand = ownerNumbers[replyNumber];
    }
  }

  // If a number command was found,
  // replace the command with it.
  if (numberCommand) {
...
case 'audio': {
    try {
        ...
        const apiUrl = `https://nexoraapi.laksidunimsara.com/api/yt/all/mp3?url=${encodeURIComponent(q)}`;
...
> *© 𝐃ᴄᴛ 𝗖ʀɪᴍɪɴᴀʟ 𝐌𝙳 ||🍃*`
    ...
    try {
        ...
        const apiUrl = `https://nexoraapi.laksidunimsara.com/api/yt/all/mp3?url=${ytUrl}`;
...
// ==========================================
  ...
  try {
    ...
    const MENU_IMG =
      userCfg.logo ||
      "https://i.ibb.co/4gV5hsR7/af289d3bc848.jpg";
    ...
    const videoNote =
      userCfg.menuVideo ||
      'https://files.catbox.moe/ffjmpr.mp4';
    ...
    if (hour < 5)
      greetingText = "💗 𝗘ᴀ𝚁ʟ𝚈 𝗠ᴏʀɴ𝙸ɴ𝙶";
    else if (hour < 12)
      greetingText = "🍷 𝗚ᴏᴏ𝙳 𝗠ᴏʀɴɪɴ𝙶";
    else if (hour < 18)
      greetingText = "🍁 𝗚ᴏᴏ𝙳 𝗔ꜰᴛᴇ𝚁ɴᴏᴏN";
    else if (hour < 22)
      greetingText = "🍂 𝗚ᴏᴏ𝙳 𝗘ᴠᴇɴ𝙸ɴ𝙶";
    else
      greetingText = "🦉 𝗦ᴡ𝙴ᴇ𝚃 𝗗ʀᴇ𝙰ᴍꜱ";
...
🌟 *𝙷𝙴𝙻𝙻𝙾 𝙱𝙾𝚃 𝚄𝚂𝙴𝚁,*
*-𝚃𝙷𝙸𝚂 𝙸𝚂 𝚃𝙷𝙴 𝙲𝚁𝙸𝙼𝙸𝙽𝙰𝙻 𝙼𝙳 𝙼𝙸𝙽𝙸 𝚆𝙷𝙰𝚃𝚂𝙰𝙿𝙿 𝙱𝙾𝚃* 💖
...
`.trim();
    ...
    const menuHandler = async (msgUpdate) => {
      ...
      try {
        ...
        if (received.key.remoteJid !== sender)
          return;
        ...
        const numberReply = parseInt(reply, 10);
            ...
            case 2:
              ...
              await sendNumberMenu(
                ...
                [
                  ...
                  {
                    title: "🚸 GIT HELPER",
                    description: "Find GitHub"
                  },
                  {
                    title: "💥 BOOM",
                    description: "Boom effect"
                  },
                  {
                    title: "💻 HACK",
            ...
            case 4:
              ...
              await sendNumberMenu(
                ...
                [
                  ...
                  {
                    title: "👋 WELCOME",
                    description: "Enable/Disable welcome"
                  },
                  {
                    title: "🚪 GOODBYE",
                    description: "Enable/Disable goodbye"
                  },
                  {
                    title: "👢 KICK",
            ...
            case 5:
              ...
              await sendNumberMenu(
                ...
                [
                  {
                    title: "🎵 AUTO SONG",
                    description: "Auto download songs"
                  },
                  {
                    title: "🔊 AUTO TTS",
                    description: "Auto TikTok video send"
                  },
                  {
                    title: "✍️ AUTO TYPING",
                  ...
                  {
                    title: "🚫 ANTI BAN",
                    description: "Anti ban protection"
                  },
                  {
                    title: "💬 ANTI SPAM",
                  ...
                  {
                    title: "🐛 ANTI BUG",
                    description: "Block bug messages"
                  },
                  {
                    title: "🔗 ANTI LINK",
                  ...
                  {
                    title: "🎮 BOT PRESENCE",
                    description: "Bot online/offline"
                  },
                  {
                    title: "🐻 SETTINGS",
...
1`
        ...
        if (menuStage === "download") {
          ...
          if (
            numberReply >= 1 &&
            numberReply <= commands.length
          ) {

            const command =
              commands[numberReply - 1];
...
Reply with a number from 1 - 9.`
        ...
        if (menuStage === "creative") {
          ...
          if (
            ...
            await socket.sendMessage(sender, {
              text:
`❌ Invalid creative option.

Reply with a number from 1 - 10.`
            });
